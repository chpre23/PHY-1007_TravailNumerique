...
class LaplaceEquationSolver:
    ...
    def _solve_in_cartesian_coordinate(
        ...
        Vk = ScalarField(constant_voltage)

        #Fonction de relaxation    
        def relaxation(Vc, ligne, colonnes):
            Vf = ScalarField(constant_voltage)
            for j in range(1, colonnes-1):
                for i in range(1, ligne-1):
                    if Vf[j, i] != 0:
                        continue
                    else:
                        Vf[j, i] = (1/4)*(Vc[j-1, i] + Vc[j+1, i] + Vc[j, i-1] + Vc[j, i+1])
            return Vf
        
        #Loop sur les itérations
        ancien_potentiel = Vk
        old_V = Vk
        new_potentiel = Vk
        for i in range(self.nb_iterations):
            old_V = new_potentiel
            new_potentiel = relaxation(ancien_potentiel, constant_voltage.shape[0], constant_voltage.shape[1])
            ancien_potentiel = old_V

        #Retourne un champ de potentiel en 2D
        return new_potentiel
    ...
    def _solve_in_polar_coordinate(
        ...
        potential : ScalarField
            ...
            the electrical compo  nents and in the empty space between the electrical components, while the field V
        ...
        dr2, dtheta2 = delta_r**2, delta_theta**2
        denom = 2*(dr2 + dtheta2)
        ...
        while error > tol and iteration < self.nb_iterations:
            ...
            for i in range(1, nr-1):
                for j in range(1, ntheta-1):
                    left, right, up, down, old_value = constant_voltage[i, j-1], constant_voltage[i, j+1], constant_voltage[i-1, j], constant_voltage[i+1, j], constant_voltage[i, j]
                    new_value = (1-w)*old_value + w/denom*((left + right)/dtheta2 + (up + down)/dr2) # P-E un facteur de r^2
